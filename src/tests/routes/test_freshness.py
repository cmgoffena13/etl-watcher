...
@pytest.mark.anyio
async def test_freshness_check_success(async_client: AsyncClient):
    """Test freshness check when pipeline passes."""
    ...
    pipeline_data = TEST_PIPELINE_FRESHNESS_DATA.copy()
...
@pytest.mark.anyio
async def test_freshness_check_muted_pipeline(async_client: AsyncClient):
...
@pytest.mark.anyio
async def test_pipeline_freshness_check_failure(
    async_client: AsyncClient, db_session: Session, mock_slack_notifications
):
    """Test freshness check failure"""
